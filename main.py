...
def main():
    while True:
        ...
        for idx, query in enumerate(queries):
            ...
            if parsed_query['select'] == "*":
                ...
                if isinstance(parsed_query['select']['value'], str):
                    ...
                    reqd_columns = [
                        {'value': parsed_query['select']['value'][val]}]
                    distinct_query = False
                    simple_query = False
            ...
            for table in tables.values():
                ...
                for reqd_col in reqd_columns:
                    if reqd_col['value'] in cols:
                        column_names.append(
                            f"{table.tablename}.{reqd_col['value']}")
        ...
        for key in table_to_cond.keys():
            table_to_cond[key] = [i for n, i in enumerate(
                table_to_cond[key]) if i not in table_to_cond[key][:n]]
            if len(table_to_cond[key]) > 1:
                table_to_cond[key].append(
                    list(parsed_query['where'].keys())[0])
        table_data = defaultdict(list)
        if where_query:
            for tab_name in table_to_col.keys():
                for tab, conditions in table_to_cond.items():
                    if tab == tab_name:
                        table_data[tab_name] = tables[tab_name].get_columns(
                            table_to_col[tab_name], conditions, parsed_query['from'])
                    else:
                        if len(table_data[tab_name]) == 0:
                            table_data[tab_name] = tables[tab_name].get_columns(
                                table_to_col[tab_name])
